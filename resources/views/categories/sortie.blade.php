<x-nav-bar>
   ...
   <div class="text-sm text-gray-900">
    avec la suppression de la sortie on annule la transation de la quantité
   </div>
                    ...
                    <tr class="bg-white  hover:bg-gray-50" href='categories.index_mar_'>
                        ...
                        <td class="py-4 px-6 justify-center space-x-4 flex ">
                            <form action="{{ route('sorties.delete', $item) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button  onclick="warnning({{ $item->id }})" class="text-red-600 hover:text-red-900">Supprimer</button>
                            </form>
